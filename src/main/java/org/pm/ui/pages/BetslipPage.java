...
public class BetslipPage extends BasePage {
  ...
  public String getSuccessAlert() {
    return $(xpath(
        "//button[@data-id='betslip-success-betslip-button']//div[contains(@class, 'SuccessBetslip__')]")).getText();
  }
  ...
  public static double getPossibleWinningAmountBySite() {
    return Double.parseDouble(
        $(xpath("//span[@data-id='betslip-place-bet-button-amount']")).getText());
  }

  public static double calculatePossibleWinningAmount(double valueBet,
  ...
  public List<Double> getOddsInBetslip() {
    ...
    for (SelenideElement outcome : $(By.xpath("//div[@data-id='betslip-container']"))
        .findAll(By.xpath(
            ".//div[contains(@class, 'Outcome__wrapper') and not(contains(@class, 'Recommended'))]"))) {
  ...
  public double getPossibleWin() {
    String possibleWinText =
        $(By.xpath("//span[@data-id='betslip-place-bet-button-amount']")).getText();
  ...
  public ArrayList<Double> getCoefficientsFromBetslip() {
    ElementsCollection coefficients = $$(xpath(
        "//div[@data-id='betslip-outcome-block']//div[@data-id='animated-odds-value']//span"));
  ...
  public static double calculatePossibleSystemWinningAmount(double valueBet,
                                                            ArrayList<Double> coefficients) {
    double result = 0;
